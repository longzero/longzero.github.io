...
$files = new RegexIterator($flattened, '/\.(?:jpg|jpeg|png|webp|avif|mp4|mov|m4v)$/i');
...
// 5. CUSTOM SORTING
// Sort by timestamp (everything after the first underscore)
// so IMG_20260101 and VID_20260101 appear together logically.
usort($images, function($a, $b) {
    $fileA = basename($a);
    $fileB = basename($b);

    // Get the part after the first underscore
    $timeA = strpos($fileA, '_') !== false ? substr($fileA, strpos($fileA, '_') + 1) : $fileA;
    $timeB = strpos($fileB, '_') !== false ? substr($fileB, strpos($fileB, '_') + 1) : $fileB;
